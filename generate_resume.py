...
class ResumePDF(FPDF):
    def header(self):
        # We only want the full header banner on the first page
        if self.page_no() == 1:
            # Top banner with name and contact
            self.set_fill_color(15, 23, 42) # Deep Slate #0f172a
            ...
            self.set_text_color(14, 165, 233) # Sky blue accent #0ea5e9
            ...
            self.cell(85, 4, 'Nairobi, Kenya', ln=1, align='R')
            self.set_x(110)
            self.cell(85, 4, 'obachi62@gmail.com', ln=1, align='R')
            self.set_x(110)
            self.cell(85, 4, '+254 798 666 374', ln=1, align='R')
            self.set_x(110)
            self.cell(85, 4, 'github.com/fred4377', ln=1, align='R')
            
            self.set_y(48) # Set position for the content to start below the header
        else:
            # Simple header for subsequent pages
            self.set_xy(15, 10)
            self.set_font('Helvetica', 'I', 8)
            self.set_text_color(156, 163, 175)
            self.cell(0, 5, 'Fred Otieno -- Resume', ln=1)
    ...
    def section_heading(self, label):
        ...
        self.set_text_color(15, 23, 42) # Deep Slate #0f172a
        self.cell(0, 8, label.upper(), ln=1)
        self.set_draw_color(14, 165, 233) # Sky Blue divider
        self.set_line_width(0.5)
        self.line(self.get_x(), self.get_y(), self.get_x() + 180, self.get_y())
        self.ln(3)

    def section_content(self, text, bold_prefix="", font_size=9.5, is_bullet=False):
        ...
        if is_bullet:
            self.cell(4, 5, chr(149), ln=0) # Bullet symbol
...
def create_resume():
    ...
    pdf.section_content(
        ...
        "Technology, with a proven track record of designing, building, and deploying 10+ dynamic web "
        "applications. Specialized in combining modern, humanized user interfaces with secure, efficient "
        "backend logic, RESTful API integrations, and localized payment features like Safaricom M-Pesa STK."
    )
    pdf.ln(4)
    ...
    pdf.cell(130, 5, 'Full-Stack Web Developer (Freelance Consultant)', ln=0)
    pdf.set_font('Helvetica', 'I', 9.5)
    pdf.set_text_color(100, 116, 139)
    pdf.cell(50, 5, '2023 - Present', ln=1, align='R')
    pdf.ln(1)
    ...
    pdf.set_font('Helvetica', 'I', 9.5)
    pdf.set_text_color(100, 116, 139)
    pdf.cell(50, 5, '2023 - 2025', ln=1, align='R')
    pdf.ln(1)
    ...
    pdf.set_font('Helvetica', 'I', 9.5)
    pdf.set_text_color(100, 116, 139)
    pdf.cell(50, 5, '2021 - 2023', ln=1, align='R')
    pdf.set_font('Helvetica', '', 9.5)
    pdf.set_text_color(71, 85, 105)
    ...
    pdf.section_content("Requirements gathering, time management, active debugging, team collaboration", bold_prefix="Soft Skills:")
